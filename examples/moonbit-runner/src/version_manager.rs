...
//!
//! .wasm バイナリをストレージディレクトリに保存し、ロールバック用に最大 N 世代保持する。
...
mod tests {
    ...
    fn save_値域確認() {
        ...
        let cases: &[(&[u8], u64)] = &[(b"data1", 1), (b"data2", 2), (b"data3", 3)];
    ...
    fn save_正常系() {
        ...
        assert_eq!(mgr.current_version(), Some(v));
    }
    ...
    #[test]
    fn current_version_正常系() {
        let (mut mgr, _dir) = tmp_manager(10);
        assert_eq!(mgr.current_version(), None, "初期状態は None");
        mgr.save(b"v1").unwrap();
        assert_eq!(mgr.current_version(), Some(1));
        mgr.save(b"v2").unwrap();
        assert_eq!(mgr.current_version(), Some(2));
        // ロールバック
        mgr.mark_current(1);
        assert_eq!(mgr.current_version(), Some(1));
    }
    ...
    fn path_of_異常系() {
        ...
        // mark_current に存在しないバージョンを渡しても無視される
        let (mut mgr2, _dir2) = tmp_manager(10);
        mgr2.mark_current(99);
        assert_eq!(mgr2.current_version(), None);
    }
}
